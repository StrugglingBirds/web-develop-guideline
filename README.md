...
11. 【推荐】前端逻辑代码处，避免写注入 `` if (isDev) {} else {} `` 等为了方面调试，将数据挂载至 window 或者在页面输出的代码，可以采用其他组件使用全局 wrapper 的方式，而不要遍地开花（本来交互逻辑，数据处理逻辑就复杂，再加上一大堆的特殊环境的判断，后期维护人员如何分得清 ？）。
